        ...
        with open(gitignore_path, "r") as gitignore_file:
            for line in gitignore_file:
                ...
                if line and not line.startswith("#"):
                    pattern = convert_gitignore_pattern(line)
                    gitignore_patterns.append(re.compile(pattern))
    return gitignore_patterns


def convert_gitignore_pattern(pattern):
...
def get_code_files(directory, patterns, ignore_patterns):
    for root, dirs, files in os.walk(directory):
        # Parse .gitignore in the current directory and update ignore_patterns
        current_ignore_patterns = ignore_patterns + parse_gitignore(root)

        # Remove ignored directories
        dirs[:] = [d for d in dirs if not should_ignore(os.path.join(root, d), directory, current_ignore_patterns)]

        for pattern in patterns:
            for filename in fnmatch.filter(files, pattern):
                file_path = os.path.join(root, filename)
                if not should_ignore(file_path, directory, current_ignore_patterns):
                    yield file_path
